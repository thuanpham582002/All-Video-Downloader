...
object UpdateUtil {
    ...
    suspend fun updateYtDlp(): String {
        withContext(Dispatchers.IO) {
            try {
                YoutubeDL.getInstance().updateYoutubeDL(App.context)
                TextUtil.makeToastSuspend(App.context.getString(R.string.yt_dlp_up_to_date))
            } catch (e: Exception) {
                TextUtil.makeToastSuspend(App.context.getString(R.string.yt_dlp_update_fail))
            }
        }
        YoutubeDL.getInstance().version(App.context)?.let {
            App.ytdlpVersion = it
            PreferenceUtil.updateString(PreferenceUtil.YT_DLP, it)
        }
        return App.ytdlpVersion
    }
